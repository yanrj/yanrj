'use strict';
angular.module('mx.contacts.groups')
/*
 * 群聊按钮
 */
 ...
 * 群聊容器
 */
.directive('groupsContacts', ['RootscopeApply', 'ContactBinder', function(RootscopeApply, ContactBinder) {
	return {
		restrict: 'EA',
 ...
 * 群聊item元素
 */
.directive('groupContactItem', ['ConvTitleFilter', function(ConvTitleFilter) {
	return {
		restrict: 'EA',
 ...
 * 群聊成员容器
 */
.directive('groupMembers', ['RootscopeApply', '$injector', 'GroupMembersServ', 'Storage',
 ...
 * 群聊成员
 */
.directive('groupMemberItem', ['GLOBAL_SETTING', 'UserInfoServ', 'GroupMemberReq', 'conversationInfoBinder',
 ...
 * 添加群聊成员按钮
 */
.directive('groupMemberAddBtn', ['GroupMemberSelect', 'conversationInfoBinder', 'ConversationBinder', '$rootScope',
 ...
 * 群聊成员选择器
 */
.directive('groupMemberSelector', ['GroupMemberSelect', '$injector', 'RootscopeApply', 'ConversationBinder', 'DepartmentsLoader', 'CompanyListBinder', 'Crumbs', 'UserSearchServ', 'PopMessage','CompanySelectedAllBinder', '$filter','Cache','$location',
	function(GroupMemberSelect, $injector, RootscopeApply, ConversationBinder, DepartmentsLoader, CompanyListBinder, Crumbs, UserSearchServ, PopMessag, CompanySelectedAllBinder, $filter, Cache,$location) {
		return {
			restrict: 'EA',
			replace: false,
			scope: true,
			template: 	'<div class="gm-selector-box {{selectorStatus}}">\
							<h1>{{ (selectorType===1 || selectorType===3) ?  "添加新成员" : "发起聊天" }}</h1>\
							<div class="friendlist-wrap details-wrap">\
								...
								<div ng-show="selectorStatus===\'friends\'" friends-list class="friendlist-box list">\
									<div class="item company-btn">\
			  							<span class="name">{{\'others.contacts_application\' | translate}}</span>\
									</div>\
								</div>\
							</div>\
			...
			controller: ['$scope', function($scope) {
				...
				$scope.addMember = function(memberItem) {
					...
					$scope.count = $scope.selectedArr.length;
					console.info('memberItem', memberItem);
					$scope.errNoPerson = false;
...
.directive('groupMemberSelectedItem', ['$rootScope',
	function($rootScope) {
		return {
			restrict: 'A',
			replace: false,
...
//全选按钮
.directive('selectedAll', ['RootscopeApply', '$injector', '$rootScope',
