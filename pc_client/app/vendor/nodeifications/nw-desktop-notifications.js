(function(){
  ...
  var counter = 0;
  var closeTimer = null; //用来关闭提醒的计时器
  ...
  function makeNewNotifyWindow() {
    var win = gui.Window.open(
      //注意，这里引用的html模板为绝对路径，如果html文件有修改，必须修改其文件名前缀的版本号，否则会有缓存
      'http://127.0.0.1:9100/vendor/nodeifications/201505281750.nw-desktop-notifications.html', {
      frame: false,
      toolbar: false,
      'always-on-top': true,
      show: false,
  ...
  function slideInNotificationWindow() {
    ...
    var startY = window.screen.availHeight;
    var y = window.screen.availHeight - window.DEA.height;
    var x = getXPositionOfNotificationWindow(win);
    var h = 0;
    var height = window.DEA.height;
    var width = window.DEA.width;

    //win.moveTo(x, startY);
    //win.resizeTo(width, h);
    win.moveTo(x, y);
