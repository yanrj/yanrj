...
var mDefaultServerAddr = "www.minxing365.com";     // 默认服务器地址
       ...
       AddEdgePage();
        ...
        setTimeout(function () {
            ...
            if (errorcode == GV_ERR_SUCCESS) {
                ...
                //登陆
                console.info('message', message ,message.creater.name);
                var loginName = message.creater.name;
                errorcode = BRAC_Login(loginName, '', 0);

                console.info('房间号:', roomID);
                //进入房间
                EnterRoomRequest(roomID);
                GetID("prompt_div").style.display = "none";         // 隐藏插件安装提示界面
                
                if(messageType == 0){
                    $('#room_div_right').addClass('MXvoice-warp');
                    $(".video-warp").hide();
                    $(".voice-warp").show();
                }else{
                    $('#room_div_right').removeClass('MXvoice-warp');
                    $(".voice-warp").hide();
                    $(".video-warp").show();
                }
                // 初始化界面元素
...
function InitInterfaceUI() {
    ...
    GetID("leaveroom").onclick = function () {
        ...
        window.parent.document.getElementById("voice-plugin").style.display="none";
        window.parent.document.getElementById('appsIframe').remove();
        var videoWarp = window.parent.document.getElementById("mx-video-ring");
        videoWarp.pause();
...
function ShowHallDiv(bShow) {
    if (bShow) {
		...
		GetID("hall_div").style.display = "none"; 		//显示大厅界面
		GetID("customroomid").value = "";
...
function RoomUserListControl(userid, bInsert) {
    ...
    console.info("$('.UserListStyle').length", $('.UserListStyle').length);
    if($('.UserListStyle').length == 1 && !loading){
        $("#leaveroom").click();
    }
    if($('.UserListStyle').length > 1 && loading){
        loading = false;
...
function onVideoScreenChange()
{
	...
	for(var i=0;i<videoCount;i++)
	{
		...
		div_videosite.className = "otherVideoWarp CLASS_VIDEOSCREEN"+videoCount;
        div_videosite.userId = useridlist[i];
        console.info('i',i);
		// 创建视频用户姓名的div
	...
	if(count<videoCount){
		for(var i=0;i<videoCount-count;i++){
			...
			GetID("div_username"+(i+count)).innerHTML="等待加入中";
		}
	}

    $(".otherVideoWarp").draggable({ 
        revert: false, 
        cursor: "move",
        scroll: false,
        zIndex: 999999,
        snap: "#room_div,.otherVideoWarp",
...
function comeToRequestedScrenn(index)
{
    ...
    console.info('comeToRequestedScrenn index', index);
...
function RequestVideoByUserId(dwUserId)
{
    ...
    $("#div_videosite"+site).attr('data-userId', dwUserId);
    console.info('$("#div_videosite"+site)',$("#div_videosite"+site).html(), dwUserId, site);
